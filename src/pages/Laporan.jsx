        ...
        const fetchLaporanData = async () => {
            try {
                ...
                if (response && response.success && response.data) {
                    setLaporanData(response.data.data || []); // Gunakan data yang dipaginate dari API
                    setTotalItems(response.data.totalItems || 0); // Pastikan total item sesuai
                } else {
                ...
                <div className="bg-gray-800 shadow-2xl rounded-lg">
                        ...
                        <table className="min-w-full divide-y divide-gray-700 table-fixed">
                            ...
                            <tbody className="bg-gray-800 divide-y divide-gray-700">
                                {currentItems.map((item, index) => (
                                    <tr key={item._id} className="hover:bg-yellow-600 hover:text-gray-900 transition duration-300">
                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-400">
                                            {index + 1 + indexOfFirstItem}
                                        </td>
                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-100">{item.reporterName}</td>
                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-400">{item.reportContent}</td>
                    ...
                    <div className="flex justify-between items-center p-6 bg-gray-700">
                        <button
                            ...
                            className={`px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                            Previous
                        </button>
                        ...
                        <button
                            ...
                            className={`px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                            Next
                        </button>
                    </div>
